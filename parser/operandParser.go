...
func (p *OperandParser) _callExpression(callee string) (Node, error) {
	...
	if callee == "bank" || callee == "namespaceValuesToArray" {
...
func (p *OperandParser) specialAsmFunctionArguments(callee string) (Node, error) {
	...
	switch callee {
	case "bank", "namespaceValuesToArray":
...
func (p *OperandParser) memberExpression() (Node, error) {
	...
	if p.lookaheadType == enumTokenTypes.DELIMITER_period {
		...
		//In case of things like .a, .x identifiers...
...
func (p *OperandParser) primaryExpression() (Node, error) {
	...
	case enumTokenTypes.IMPLICIT_UNDEFINED:
		return p.implicitUndefined()
...
// [[[[[[[[[[[[[[[[
func (p *OperandParser) arrayExpression() (Node, error) {
	err := p.eatFreelyAndAdvance(enumTokenTypes.DELIMITER_leftSquareBracket)
	if err != nil {
		return p.badEat(err) // ❌ Fails
	}
	...
	arrayNode := operandFactory.CreateArrayNode(argumentList)

	if p.lookaheadType == enumTokenTypes.DELIMITER_leftSquareBracket {
		indexNodes := []Node{}
		err := p.computedMemberExpressionIndexes(&indexNodes)
		if err != nil {
			return arrayNode, err // ❌ Fails
		}
		return operandFactory.CreateMemberExpressionNode("_", "_", arrayNode, true, indexNodes), nil
	} else {
...
// -----------------
func (p *OperandParser) implicitUndefined() (Node, error) {
	err := p.eatFreelyAndAdvance(enumTokenTypes.IMPLICIT_UNDEFINED)
	if err != nil {
		return p.badEat(err) // ❌ Fails
	}
	return operandFactory.CreateImplicitUndefinedNode(), nil
}
