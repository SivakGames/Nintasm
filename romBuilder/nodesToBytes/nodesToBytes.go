...
func ConvertNodeValueToUInts(node Node, operandByteSize int, isBranch bool, isBigEndian bool, isExact bool) ([]uint8, error) {
	...
	case enumNodeTypes.ImplicitUndefined:
		return convertedValue, errorHandler.AddNew(enumErrorCodes.ResolvedValueIsImplicitUndef)

	default:
		panic("🛑 Something is very wrong with operand conversion!")
	}

	//Arrays are technially OK, but have to fit
