...
var assemblerBuiltInFunctions = map[string]assemblerFunction{
	...
	"tan":      {1, 1, true, []enumNodeTypes.Def{enumNodeTypes.NumericLiteral}},
	"tandeg":   {1, 1, true, []enumNodeTypes.Def{enumNodeTypes.NumericLiteral}},
	...
	//Array-related
	"arrayLen":      {1, 1, true, []enumNodeTypes.Def{enumNodeTypes.Array}},
	"arrayByteLen":  {1, 1, true, []enumNodeTypes.Def{enumNodeTypes.Array}},
	"subArray":      {2, 3, true, []enumNodeTypes.Def{enumNodeTypes.Array, enumNodeTypes.NumericLiteral, enumNodeTypes.NumericLiteral}},
	"arrayContains": {2, 2, true, []enumNodeTypes.Def{enumNodeTypes.Array, enumNodeTypes.NumericLiteral}},
	"arrayIndex":    {2, 2, true, []enumNodeTypes.Def{enumNodeTypes.Array, enumNodeTypes.NumericLiteral}},
	...
	"namespaceValuesToArray": {1, 1, false, []enumNodeTypes.Def{enumNodeTypes.Identifier}},
}
...
func processAssemblerFunction(node *Node) error {
	...
	case "tan":
		node.AsNumber = math.Tan(evaluatedArguments[0].AsNumber)
	case "sindeg":
	...
	case "tandeg":
		node.AsNumber = math.Tan(evaluatedArguments[0].AsNumber * math.Pi / 180)
	...
	case "toCharmap":
		...
		charsAsArray, err := charmapTable.MapStringToCharmap(nodeString)
		if err != nil {
			return err
		}
		operandFactory.ConvertNodeToArray(node, charsAsArray)

	// NON pre-evaluated functions!
	case "namespaceValuesToArray":
		...
		operandFactory.ConvertNodeToArray(node, nsValuesAsNode)
	...
	case "defined":
		...
		if baseNode.Resolved {
			node.AsBool = true
			if baseNode.NodeType == enumNodeTypes.ImplicitUndefined {
				node.AsBool = false
			}
	...
	case
		...
		"ceil", "floor", "round",
		"modfDeci", "modfInt",
		"sin", "sindeg", "cos", "cosdeg", "tan", "tandeg",
		"bank",
		"strLen", "arrayByteLen", "arrayLen",
		"arrayIndex":
		operandFactory.ConvertNodeToNumericLiteral(node)
	...
	case "arrayContains":
		operandFactory.ConvertNodeToBooleanLiteral(node)
	case "subArray":
		operandFactory.ConvertNodeToArray(node, *node.ArgumentList)
	}
