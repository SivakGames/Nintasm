...
func PrintTotalErrorMessage(warningOnly bool) {
	fmt.Println()
	if warningOnly {
		fmt.Println(util.Colorize("Assembly has succeeded, but with warnings", enumTerminalColors.Yellow, false))
	} else {
		fmt.Println(util.Colorize("Assembly has terminated with errors!", enumTerminalColors.AnsiOrange, false))
	}
